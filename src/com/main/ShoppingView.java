...
public class ShoppingView extends View {
	...
	private void init(AttributeSet attrs) {
		...
		mDuration = typeArray.getInt(R.styleable.ShoppingView_sv_duration,
				DEFAULT_DURATION);
		mShoppingText = TextUtils.isEmpty(typeArray
				.getString(R.styleable.ShoppingView_sv_text)) ? DEFAULT_SHOPPING_TEXT
				: typeArray.getString(R.styleable.ShoppingView_sv_text);
		// 展示锟侥帮拷锟斤拷小
		int textSize = (int) typeArray.getDimension(
				R.styleable.ShoppingView_sv_text_size, sp2px(16));
		// 锟斤拷锟斤拷色
	...
	private void drawBgMove(Canvas canvas) {
		canvas.drawArc(new RectF(mWidth, 0, mWidth + MAX_HEIGHT, MAX_HEIGHT),
				90, 180, false, mPaintBg);
		canvas.drawRect(new RectF(mWidth + MAX_HEIGHT / 2, 0, MAX_WIDTH
				- MAX_HEIGHT / 2, MAX_HEIGHT), mPaintBg);
		canvas.drawArc(new RectF(MAX_WIDTH - MAX_HEIGHT, 0, MAX_WIDTH,
				MAX_HEIGHT), 180, 270, false, mPaintBg);
	}
	...
	private void drawShoppingText(Canvas canvas) {
		canvas.drawText(mShoppingText,
				MAX_WIDTH / 2 - getTextWidth(mPaintText, mShoppingText) / 2f,
				MAX_HEIGHT / 2 + getTextHeight(mShoppingText, mPaintText) / 2f,
				mPaintText);
	}
	...
	private void drawAddBtn(Canvas canvas) {
		canvas.drawCircle(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 2,
				MAX_HEIGHT / 2, mPaintBg);
		canvas.drawLine(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 4, MAX_WIDTH
				- MAX_HEIGHT / 2, MAX_HEIGHT / 4 * 3, mPaintText);
		canvas.drawLine(MAX_WIDTH - MAX_HEIGHT / 2 - MAX_HEIGHT / 4,
				MAX_HEIGHT / 2, MAX_WIDTH - MAX_HEIGHT / 4, MAX_HEIGHT / 2,
				mPaintText);
	}
	...
	private void drawMinusBtn(Canvas canvas, float angle) {
		...
		canvas.drawCircle(mMinusBtnPosition, MAX_HEIGHT / 2, MAX_HEIGHT / 2
				- MAX_HEIGHT / 20, mPaintMinus);
	...
	private void drawNumText(Canvas canvas) {
		drawText(canvas, String.valueOf(mNum),
				mTextPosition - getTextWidth(mPaintNum, String.valueOf(mNum))
						/ 2f,
				MAX_HEIGHT / 2 + getTextHeight(String.valueOf(mNum), mPaintNum)
						/ 2f, mPaintNum, mAngle);
	}
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mState == STATE_NONE) {
				...
				if (isPointInCircle(new PointF(event.getX(), event.getY()),
						new PointF(MAX_WIDTH - MAX_HEIGHT / 2, MAX_HEIGHT / 2),
						MAX_HEIGHT / 2)) {
					if (mNum > 0) {
						mNum++;
						mIsForward = true;
					...
					invalidate();
						...
						new PointF(event.getX(), event.getY()), new PointF(
								MAX_HEIGHT / 2, MAX_HEIGHT / 2), MAX_HEIGHT / 2)) {
					if (mNum > 1) {
						mNum--;
						if (mShoppingClickListener != null) {
	...
	private boolean isPointInCircle(PointF pointF, PointF circle, float radius) {
		return Math.pow((pointF.x - circle.x), 2)
				+ Math.pow((pointF.y - circle.y), 2) <= Math.pow(radius, 2);
	}
